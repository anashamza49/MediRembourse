<?php
    require "db.php";
    $stmt = $pdo->prepare(
        "SELECT * FROM demandes WHERE etat = 'Approuvée'"
    );
    $stmt->execute();
...
<head>
    ...
    <title>Demandes Approuvées</title>
                    ...
                    <?php foreach ($result as $row) : ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <a class="action-button" href="rejeterDemande.php?id=<?= $row["id"] ?>" class="reject-link" data-id="<?= $row["id"] ?>">Rejeter</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            rejectLinks.forEach(link => {
                // Ajout d'un écouteur d'événement "click" à chaque lien
                link.addEventListener("click", function(event) {
                    event.preventDefault();
                    const demandId = this.getAttribute("data-id");
                    const rejectionReason = prompt("Raison du rejet :");
                    // Vérification si une raison de rejet a été fournie
